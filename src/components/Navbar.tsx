...
export const Navbar: React.FC = () => {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => audioHaptics.isEnabled());
  ...
  const triggerBrandEasterEgg = (e: React.MouseEvent) => {
    e.preventDefault();
    audioHaptics.playCrunch();

    // Trigger microchip burst confetti
    confetti({
      particleCount: 45,
      spread: 60,
      origin: { x: 0.12, y: 0.08 },
      colors: [themeConfig.primaryColor, themeConfig.accentColor, '#ffd700', '#ffffff'],
      ticks: 120,
    });

    setEasterEggToast('bagOfchips SES · Engineered with full IP sovereignty worldwide');
    setTimeout(() => {
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 pt-4 sm:pt-6 px-4 pointer-events-none">
        <div className="max-w-5xl mx-auto flex items-center justify-between pointer-events-auto">
          ...
          >
            {/* Logo with bagOfchips SES Icon & Easter Egg */}
            <a
              href="#"
              onClick={triggerBrandEasterEgg}
              className="flex items-center gap-2.5 group cursor-pointer select-none"
              title="Click to unleash the chips!"
            >
              <div className="relative w-8 h-8 rounded-xl overflow-hidden border border-white/15 bg-slate-900 shadow-md group-hover:scale-110 group-hover:border-white/30 transition-all duration-300 shrink-0">
                <img
                ...
                />
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-25 transition-opacity pointer-events-none rounded-xl"
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {/* Web Audio Haptics Sound Toggle */}
              <button
              ...
              <a
                href="#contact"
                onClick={() => audioHaptics.playClick(1800)}
                className="px-4 py-1.5 rounded-full text-xs font-medium text-white transition-all duration-200 hover:opacity-90 active:scale-95 cursor-pointer flex items-center gap-1.5 shadow-md"
                style={{
      ...
      {easterEggToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-none animate-in fade-in slide-in-from-bottom-3 duration-300">
          <div className="px-4 py-2.5 rounded-full bg-slate-900/95 border border-white/20 text-xs text-white shadow-2xl backdrop-blur-xl flex items-center gap-2">
